...
module.exports = (options, client) => {
	...
	if (!options.embed.color) options.embed.color = "#2f3136"
	...
	if (!options.disabledQuery) options.disabledQuery = options.author.username+', calculator has been disabled!'
	...
	if (!options.othersMessage) {
		options.othersMessage = '**{{author}}**, sorry only game commander can use the buttons!'
	}
	if (typeof options.othersMessage !== 'string') {
		throw new TypeError('Error: othersMessage must be a string.');
	}

	if (data.has(options.author.id)) return;
	data.add(options.author.id);
	...
	const embed = new Discord.MessageEmbed()
		.setAuthor(client.user.username, "", "https://discord.com/")
		.setFooter(options.author.username, options.author.avatarURL())
	...
	options.inlineReply(embed).then(async (msg) => {
		msg.edit({
			embed: embed,
			components: [
				{
		...
		calc.on('collect', async (btn) => {
			if (btn.clicker.user.id !== options.author.id) {
				return btn.reply.send(
					options.othersMessage.replace(
						'{{author}}',
						btn.clicker.user.username,
					),
					true,
				);
			}
